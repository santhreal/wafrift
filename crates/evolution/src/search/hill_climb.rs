    ...
    fn restore(&mut self, bytes: &[u8]) -> Result<(), EvolutionError> {
        if bytes.len() > crate::types::MAX_CHECKPOINT_BYTES {
            return Err(EvolutionError::OversizedData {
                context: "hill_climb checkpoint restore".into(),
                size: bytes.len(),
...
mod tests {
    ...
    /// Over many rounds, `eval_counter`-derived IDs must be strictly increasing
    /// (no reuse that could cause the engine's `in_flight` map to collide).
    #[test]
