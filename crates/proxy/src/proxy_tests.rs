...
async fn proxy_state_escalation_levels() {
    use wafrift_types::EscalationLevel;
    let mut state = HostState::default();
...
fn pipeline_state_per_host_isolated() {
    ...
    assert_eq!(
        state
            ...
            .unwrap()
            .escalation_level(),
        wafrift_types::EscalationLevel::Medium
    );
    assert_eq!(
        state
            .hosts
            ...
            .unwrap()
            .escalation_level(),
        wafrift_types::EscalationLevel::None
    );
}
...
fn restore_gene_bank_enforces_10k_cap() {
    ...
    assert_eq!(restored, 10_000, "F141 caps inserts at MAX_RESTORED_HOSTS");
    assert!(
